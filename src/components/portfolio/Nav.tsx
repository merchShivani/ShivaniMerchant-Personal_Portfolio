  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-navy/10 bg-paper/90 backdrop-blur-md">
      <nav className="mx-auto grid h-16 max-w-[1600px] grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-5 md:flex md:justify-between md:px-8">
        ...
        <ul className="hidden items-center gap-7 md:flex">{links.map((link) => <li key={link.id}><a href={`#${link.id}`} className="text-[11px] font-semibold text-navy transition-colors hover:text-coral">{link.label}</a></li>)}<li><Link to="/linktree" className="rounded-full bg-coral px-4 py-2 text-[11px] font-bold text-paper shadow-sm transition-transform hover:-translate-y-0.5">Quick Links ✦</Link></li></ul>
      ...
      {open ? <div className="border-t border-navy/10 bg-paper px-5 py-4 md:hidden"><ul className="grid grid-cols-2 gap-2">{links.map((link) => <li key={link.id}><a href={`#${link.id}`} onClick={() => setOpen(false)} className="block rounded-md bg-pink/50 px-4 py-3 text-sm font-bold text-navy">{link.label}</a></li>)}<li className="col-span-2"><Link to="/linktree" onClick={() => setOpen(false)} className="block rounded-md bg-coral px-4 py-3 text-center text-sm font-bold text-paper">Quick Links ✦</Link></li></ul></div> : null}
    </header>
  );
}
